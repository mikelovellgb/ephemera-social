...
async fn test_fixtures_are_valid() {
    ...
    assert!(ephemera_crypto::identity::verify_signature(
        pseudo.identity_key().as_bytes(),
        msg,
        &sig
    )
    .is_ok());
    ...
    let req = fixtures::random_connection_request(&from.identity_key(), &to.identity_key());
